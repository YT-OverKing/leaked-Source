...
local ATS = Tab:AddSwitch("Auto Stats", function(bool)
    ...
    while _G.on2 do
        wait()
        local A = 1
        game:GetService("ReplicatedStorage").Remotes[StatsDropDownValue]:FireServer(A)
    end
end)
ATS:Set(false)
...
local WeaponDD;
local We = Tab:AddDropdown("Weapon Select",function(We)
    WeaponDD = We
end)
We:Add("GhastSword")
We:Add("CursedGlove")
...
tween = tweenService:Create(game:GetService("Players")["LocalPlayer"].Character.HumanoidRootPart, tweenInfo, {CFrame = CFrame.new(Workspace.NPCs[StatsDropDownValue2].HumanoidRootPart.Position + Vector3.new(0,-5,0),Workspace.NPCs[StatsDropDownValue2].HumanoidRootPart.Position)})
tween:Play()
                        end) 
						-- Attack
...
game:GetService("Workspace")[plr.Name].Main.Swing:FireServer()
    end)
end
spawn(function()
    while wait() do
...
ATQ:Add("Kill Criminal")
ATQ:Add("Kill Police")
ATQ:Add("Kill Weak Villain")
ATQ:Add("Defeat UA Student")
ATQ:Add("Kill Villain")
ATQ:Add("Kill Hero")
ATQ:Add("Kill Pro Hero")
ATQ:Add("Kill Weak Nomu")
ATQ:Add("Defeat Forest Beast")
ATQ:Add("Kill High End")

local ATQ2 = Tab:AddSwitch("Auto Quest", function(bool)
_G.ATQ = bool
while _G.ATQ do
wait()
local plr = game.Players.LocalPlayer
if plr.PlayerGui.QuestGui.QuestObjectives.KilledAmount.Text == "15/15" then
game.ReplicatedStorage.Remotes.Quest.CompleteQuest:FireServer(StatsDropDownValue3)
elseif plr.PlayerGui.QuestGui.QuestObjectives.KilledAmount.Text ~= "15/15" then
game.ReplicatedStorage.Remotes.Quest.AcceptQuest:FireServer(StatsDropDownValue3)
end
end
end)
ATQ2:Set(false)
Tab:AddLabel('---Other---')
local ANti = Tab:AddSwitch("Auto Daily", function(bool)
_G.Da = bool
while _G.Da do
	wait()
...
ANti:Set(false)
    local Order = {14,35,31,7,34,2,3,17,29,20,21,27,33,22,4,28,12,19,30,8,23,24,13,9,16,1,10,15,6,18,26,25,32,11,5}
    local UserInputService = game:GetService("UserInputService")
    local HttpService = game:GetService("HttpService")
    local RunService = game:GetService("RunService")
    local PlayerData = game.Workspace:FindFirstChild("S1c2R5i66p5t5s51").PlayerData
    ...
    local AntiRPTG = false
local AntiRp = Tab:AddSwitch("Anti Report", function(bool)
	AntiRPTG = bool
end)
...
local ANti = Tab:AddSwitch("AntiAFK", function(bool)
_G.ANti = bool
while _G.ANti do
wait()
...
ANti:Set(false)
local TabSKill = Window:AddTab("Skill")

local SKILLE = TabSKill:AddSwitch("Skill E", function(bool)
                Toggle = bool
                while wait() do
...
local SKILLZ = TabSKill:AddSwitch("Skill Z", function(bool)
                Toggle = bool
                while wait() do
                    if Toggle == false then break end
                                for i,v in pairs(LocalPlayer.Character:GetChildren()) do
...
local SKILLX = TabSKill:AddSwitch("Skill X", function(bool)
                Toggle = bool
                while wait() do
                    if Toggle == false then break end
                                for i,v in pairs(LocalPlayer.Character:GetChildren()) do
...
local SKILLC = TabSKill:AddSwitch("Skill C", function(bool)
                Toggle = bool
                while wait() do
                    if Toggle == false then break end
                                for i,v in pairs(LocalPlayer.Character:GetChildren()) do
...
local SKILLV = TabSKill:AddSwitch("Skill V", function(bool)
                Toggle = bool
                while wait() do
                    if Toggle == false then break end
                                for i,v in pairs(LocalPlayer.Character:GetChildren()) do
		...
		local TabS = Window:AddTab("Spin (Soon)")
...
local Typelist = {	
		...
		local SelectWTF
		local QuirkDD = TabS:AddDropdown('Quirk', function(quirk)
		SelectWTF = quirk
		end)
		for i, v in pairs(Quirklist) do 
		QuirkDD:Add(tostring(v))
		end
		local TypWTF
		local TypeWTF = TabS:AddDropdown('Type', function(type)
		TypWTF = type
		end)
		for i, v in pairs(Typelist) do 
		TypeWTF:Add(tostring(v))
		end
		
		local ATSPIN = false
		TabS:AddSwitch('Auto Spin (Soon)', function(bool)
		ATSPIN = bool
		end)
function autospin()
    pcall(function()
